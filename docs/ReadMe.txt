...
Obviously, one result of these statistics will be to create a truly intelligent system for calculating the most frequently, most recently, most constantly, most whatever apps.  This system is much more desirable than Haiku's current implimentation of only the most recent apps.  Also, all the statistics for each file are saved as attributes, so are freely available to anyone who wishes to use them.  At this point, there is only one "rank" calculated by the engine, but this may change with time.

The engine has some functions for returning a list of the highest ranking apps.   The first function will simply print the list out to the terminal.  Use the option --rank in a terminal to get the printed list.  You can also request the list from other applications via BMessages.  To do this create a BMessage with the command E_REPLY_RANKING_APPS, as defined in the engine_constants.h file.  If you wish to receive a certain number of apps, add an INT16 called "count" to the message.  If "count" is not specified you will receive the entire list of apps.  Then just send this message to the engine.  When the message is received, a list of the "count" highest ranking apps will be added to a reply message.  Each application will be added to the message as an entry_ref using the AddRef function with an array name of "refs".  The highest ranking app will be the first element in the array (index 0), and each next highest ranking app will be at the next index, down to the lowest ranking app for as many apps as you specified in "count".  This message will be sent back to your app as a reply with the command E_RANKING_APPS_REPLY, again as defined in engine_constants.h.
